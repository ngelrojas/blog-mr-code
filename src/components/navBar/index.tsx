...
export default function NavBar() {
    ...
    const iconMode = (dark: boolean) => {
        if (dark) {
            return (<svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5"
                         stroke="currentColor" className="w-6 h-6 bg-header text-white">
    ...
    return (
        <Disclosure as="nav"
                    className={`fixed container max-w-full z-50 ${theme === 'light' ? 'border-b border-gray-300 bg-white text-black' : 'bg-header text-white'}`}>
            {({open}) => (
                <>
                    <div
